        ...
        <form method="POST" action="{{ route('tenant.tasks.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}"
                       ...
                       class="block w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('title') border-red-500 @enderror"
                       required>
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          ...
                          class="block w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
            </div>
                    ...
                    <select id="assigned_to" name="assigned_to"
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('assigned_to') == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                                @if($user->id === auth('tenant')->id()) (You) @endif
                            </option>
                        @endforeach
                    </select>
                </div>
