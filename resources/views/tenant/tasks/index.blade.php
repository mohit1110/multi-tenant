...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
    <div class="flex flex-wrap items-center gap-3 px-5 py-4">
        <form method="GET" action="{{ route('tenant.tasks.index') }}" class="flex flex-wrap gap-3 flex-1">
            ...
            @if(request()->hasAny(['search', 'status', 'priority']))
                <a href="{{ route('tenant.tasks.index') }}" class="px-4 py-2 text-gray-500 text-sm hover:text-gray-700">
                    Clear
                </a>
            @endif
        </form>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-sm">
                        @if($task->due_date)
                            <span class="{{ $task->due_date->isPast() && !$task->isCompleted() ? 'text-red-600 font-medium' : 'text-gray-600' }}">
