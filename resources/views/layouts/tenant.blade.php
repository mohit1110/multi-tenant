...
<div class="min-h-full flex">
    ...
    <div id="sidebar" class="max-md:hidden transition max-h-screen w-64 bg-white shadow-sm flex flex-col fixed inset-y-0 left-0 z-50">
        ...
        <div class="flex items-center justify-between h-16 px-6 border-b border-gray-200">
          <div class="flex items-center gap-2">
            ...
            <div>
                <p class="text-sm font-bold text-gray-900">{{ $tenant->name?? config('app.name') }}</p>
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 *:flex *:gap-2">
            ...
            <a href="{{ route('tenant.tasks.index') }}"
               class="sidebar-link {{ request()->routeIs('tenant.tasks.*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-4 py-4 border-t border-gray-200">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-8 h-8 bg-indigo-600 rounded-full flex items-center justify-center text-white text-sm font-bold">
                    {{ strtoupper(substr(auth('tenant')->user()->name, 0, 1)) }}
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900 truncate">{{ auth('tenant')->user()->name }}</p>
                    <p class="text-xs text-gray-500 truncate">{{ auth('tenant')->user()->email }}</p>
                </div>
            </div>
            <form method="GET" action="{{ route('tenant.logout') }}">
                @csrf
                <button type="submit"
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
            ...
            <div class="flex items-center gap-4">
                @if(auth('tenant')->user()->isTenantAdmin())
